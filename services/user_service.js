...
module.exports.addNewUser = async function (user) {
  if (!user || !user.password || !user.name) throw new HttpRequestParamError('user', '用户名或密码不能为空', 'empty user or password');
...
module.exports.loginWithNamePass = (username, password) => {
  if (!username || !password) throw new HttpRequestParamError('user', '用户名或密码不能为空', 'empty user or password');
  ...
  const token = JWT.sign({
    ...
    expireAt: Date.noew().valueOf() + JWTConfig.expireIn,
  }, JWTConfig.SECRET);
  return {
    token,
    user: found,
